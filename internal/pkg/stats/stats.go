...
func (c *CollectionStats) StartCollectionStats(ctx context.Context) {
	...
	go func() {
		...
		for {
			...
			if iter%4 == 0 {
				cols, err := mdb.ListDbCollections(ctx, config.Current.Repl.Databases)
				if err != nil {
...
func (c *CollectionStats) getCollectionStats(db string, collections []string) {
	for _, collection := range collections {
		...
		count, err := mdb.GetCollectionStats(mdb.Registry.GetSource(), db, collection)
		if err != nil {
			continue
		}
		...
		count, err = mdb.GetCollectionStats(mdb.Registry.GetTarget(), db, collection)
		if err != nil {
			continue
		}
